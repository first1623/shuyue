...
class KnowledgeGraphHandler(BaseHTTPRequestHandler):
        ...
        try:
            if self.path == "/":
                self._send_json_response({
                    ...
                    "status": "running",
                    "python_version": "3.13",
                    "mode": "builtin_http_server_fixed",
                    "endpoints": {
                        "GET /": "系统信息",
                        "GET /health": "健康检查", 
                ...
                self._send_json_response({
                    ...
                    "python_version": "3.13",
                    "server": "builtin_http_server_fixed"
                })
                ...
                # 修复：正确提取数字ID
                match = re.search(r'/nodes/(\d+)', self.path)
                if match:
                    ...
                    self._send_error(400, "无效的节点ID格式")
            ...
            self._send_error(400, f"参数格式错误: {str(ve)}")
    ...
    def do_POST(self):
        ...
        try:
            # 修复：正确解析文档解析接口
            if '/api/v1/documents/' in self.path and self.path.endswith('/parse'):
                # 提取文档ID
                match = re.search(r'/documents/(\d+)/parse', self.path)
                if match:
                    ...
                    self._send_error(400, "无效的文档ID格式")
            
            elif self.path == "/api/v1/knowledge-tree/scan":
                self._send_json_response({
                    "code": 200,
                    "message": "扫描任务已开始",
                    "data": {
...
def run_server(port=8000):
    ...
    print("🎉 学习平台知识图谱系统 - 修复版服务器")
    print("="*60)
    print(f"📍 Python版本: {os.sys.version}")
    print(f"🌐 服务地址: http://localhost:{port}")
    print(f"💡 运行模式: 纯Python标准库 (修复版)")
